 ...
 * cc -I/usr/local/include -o eventBus-read-fifo eventBus-read-fifo.c \
...
fifo_read(evutil_socket_t fd, short event, void *arg)
{
	...
	fprintf(stderr, "fifo_read called with fd: %d, eventBus: %d, arg: %p\n",
	    (int)fd, event, arg);
#ifdef _WIN32
...
/* On Unix, cleanup eventBus.fifo if SIGINT is received. */
#ifndef _WIN32
static void
...
#endif
	/* Initialize the eventBus library */
	base = event_base_new();

	/* Initialize one eventBus */
#ifdef _WIN32
	evfifo = event_new(base, (evutil_socket_t)socket, EV_READ|EV_PERSIST, fifo_read,
                           event_self_cbarg());
#else
	/* catch SIGINT so that eventBus.fifo can be cleaned up */
